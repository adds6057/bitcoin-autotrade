...
# 시작 시 목표가 계산 및 알림
initial_target_price = get_target_price("KRW-BTC", 0.5)
if initial_target_price is not None:
    post_message(myToken, "#btcauto", f"프로그램 시작 - 오늘의 목표 매수가: {initial_target_price:,.0f}원")
...
while True:
    try:
        ...
        # 3시간마다 상태 메시지 전송
        if last_status_message_time is None or (now - last_status_message_time).total_seconds() >= 10800:
            post_message(myToken, "#btcauto", "자동매매 프로그램 정상 작동중입니다.")
            last_status_message_time = now
